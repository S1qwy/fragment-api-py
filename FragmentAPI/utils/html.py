...
def parse_owner_history(html: str) -> tuple[list[OwnerHistoryEntry], str | None]:
    ...
    owners = [
        OwnerHistoryEntry(
            ownership_type=e.get("price_label") or e["price"],
            date=e["date"],
            wallet=e["wallet"],
        )
        for e in entries
...
def parse_stars_packages(html: str) -> list[StarsPrice]:
    ...
    for match in re.finditer(
        r'value="(\d+)".*?icon-ton">(\d*)'
        r'(?:<span class="mini-frac">\.(\d+)</span>)?</div>.*?'
        r'&#036;(\d+(?:,\d+)*(?:\.\d+)?)',
        html,
        re.DOTALL
    ):
        stars, int_part, frac, usd = match.groups()
        ton_price = f"{int_part}.{frac}" if frac else (int_part or "0")
        packages.append(StarsPrice(
            stars=int(stars),
            ton_price=ton_price,
            usd_price=usd.replace(',', '')
        ))
